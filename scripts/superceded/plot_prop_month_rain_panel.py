...
fopen = nc.Dataset("/data2/Kerryn/AGCD_Clim_Sarah_Sapsford_Paper/masked_prop_change_1993_2022_minus_1961_1990.nc", "r")
...
sig_file = "/data2/Kerryn/AGCD_Clim_Sarah_Sapsford_Paper/mask_prop_month_significance_masked.nc"
...
fig.suptitle("Proportional change in monthly rainfall", fontsize=14, y=0.95)
print("Adding title")
...
for i in range(precip.shape[0]):
    ...
    cs = axs[i].contourf(lon[:], lat[:], month_precip, transform=ccrs.PlateCarree(),
                         cmap=cm, levels=np.linspace(-1, 1, n_bins), extend='both')

    # Overlay cross hatching (X pattern) for significant areas using grey workaround
...
cbar = fig.colorbar(cs, ax=axs, orientation='horizontal', fraction=0.02, pad=0.04, ticks=np.arange(-1, 1.1, 0.2))
precip_units = 'mm'
