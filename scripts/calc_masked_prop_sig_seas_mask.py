...
hist_safe = np.where(hist_seasonal == 0, epsilon, hist_seasonal)
prop_change = (recent_seasonal - hist_seasonal) / hist_safe
...
for i in range(n_seasons):
    prop_vals = prop_change[i].reshape(n_years, -1)
...
change = 'prop'
mask_label = 'mask'
output_filename = f"{mask_label}_{change}_{time}_significance_masked_{version}.nc"
...
sig_ds["significance"].attrs["description"] = "Binary mask where 1 indicates p < 0.05 from Mann-Whitney U test on proportional change, masked by valid regions"
